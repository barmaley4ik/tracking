<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">
      <inertia-link class="text-indigo-400 hover:text-indigo-600" href="/admin/containers">Containers</inertia-link>
      <span class="text-indigo-400 font-medium">/</span>Editing
    </h1>
    <div class="bg-white rounded shadow overflow-hidden max-w-3xl">
      <form @submit.prevent="submit">
        <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
          <div class="flex w-full mr-6 mb-6">
            <text-input v-model.trim="form.name" :errors="$page.errors.name" class="pb-4 w-full" label="Conrainer" />
          </div>
          ...
          <div class="bg-white border-none w-full px-4  text-md flex flex-wrap justify-around items-center mr-6">
            <button v-for="type in typeImages" :key="`button2-`+type.name" class="mb-2 w-50 h-10 rounded p-2 text-white cursor-pointer hover:bg-indigo-300" :class="(imageField == type.name) ? 'bg-red-600' :'bg-indigo-600'" :value="type.name" @click.stop.prevent="imageField=type.name">{{ type.title }}</button>
          </div> 
          ...
          <template v-for="(image, imageindex) in form.image_upload">
            <div v-show="imageField=='image_upload'" :key="`image_upload`+imageindex" class="flex w-full flex-wrap mr-2 justify-between items-center mb-8">
              ...
              <div class="flex flex-col w-4/7 mr-2">
                ...
                <button title="Delete" :class="image ? 'pt-2 px-4 w-0' : 'invisible'" @click.stop.prevent="form.image_upload.splice(imageindex, 1)">
                  <icon name="trash" class="block w-4 h-4 fill-red-600" />
                </button>
              </div>                
              <button title="Select image" class="inline-flex bg-red-600 text-white w-14 h-10 rounded mt-6 cursor-pointer w-auto ml-5 pr-2 items-center" @click.stop.prevent="open = true; imageField='image_upload'; imageFieldIndex=imageindex">
          ...
          <template v-for="(image, imageindex) in form.image_download">
            <div v-show="imageField=='image_download'" :key="`image_download`+imageindex" class="flex w-full flex-wrap justify-between items-center mb-8 mr-6">
              <img :src="image ? image : noImage" class="h-16 object-contain">
              <div class="flex flex-col w-4/7 mr-2">
                <div class="pr-2 text-md">Image:</div>
                <div class="pt-2 text-sm w-5/7 break-all mx-4">{{ image ? image : 'Nothing selected' }}</div>
                <div v-if="$page.errors.image" class="form-error">{{ $page.errors.image[0] }}</div>
                <button title="Delete" :class="image ? 'pt-2 px-4 w-0' : 'invisible'" @click.stop.prevent="form.image_download.splice(imageindex, 1)">
                  <icon name="trash" class="block w-4 h-4 fill-red-600" />
                </button>
              </div>                
              <button title="Select image" class="inline-flex bg-red-600 text-white w-14 h-10 rounded mt-6 cursor-pointer w-auto pr-2 items-center" @click.stop.prevent="open = true; imageField='image_download'; imageFieldIndex=imageindex">
                <svg class="fill-current text-white w-6 h-6 ml-2 mr-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                  <path d="M0 4c0-1.1.9-2 2-2h16a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V4zm11 9l-3-3-6 6h16l-5-5-2 2zm4-4a2 2 0 1 0 0-4 2 2 0 0 0 0 4z" /></svg>
                ...
              </button>                 
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        image_upload: this.container.image_upload,
        image_download:this.container.image_download,
      },
      progress: 0,
      progressText: '',
      zipFilename: '',  
      typeImages: [{title:'Photos loading cars', name: 'image_download'}, {title:'Photos unloading cars', name: 'image_upload'}],      
      imageField: 'image_download',
      imageFieldIndex: '',
      error_msg: {},
      noImage: this.$page.image.noImagePath,
      homeDirectory: 'containers',
      folders: Array,
      files: Array,
      items: Object,
      links: [],
  ...
  methods: {
    submit() {
      this.sending = true
      this.$inertia.put('/admin/containers/'+ this.container.id, this.form)
 ...
 sendingEventZip(file, xhr, formData) {
    ...
    async vsuccessZip(file, success) {
      ...
      const image_fields = ['loading', 'unloading']
      ...
      if(image_fields.includes(arrFilename[0])) {
          switch(arrFilename[0]) {
            case 'loading':  
            optimizeResponse.data.files.forEach(element => (this.form.image_download).push(element))
            this.imageField = 'image_download'
            break

            case 'unloading':  
            optimizeResponse.data.files.forEach(element => (this.form.image_upload).push(element))
            this.imageField = 'image_upload'
            break
          }
      }
    ...
    async unzip(file) {
      return this.axios.get('/admin/fm/unzip'+'?type=containers'+'&zipFilename='+file);
    }, 
    async optimize(files) {
      return this.axios.get('/admin/fm/optimize'+'?type=containers'+'&listFiles='+files);
    },
