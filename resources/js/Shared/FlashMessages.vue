<template>
  <div>
    <div v-if="$page.flash.success && show" class="mb-8 flex w-full items-center justify-between bg-green-500 rounded">
    ...
    <div v-if="($page.flash.error || Object.keys($page.errors).length > 0) && show" class="mb-8 flex items-center justify-between bg-red-500 rounded ">
      <div class="flex items-center">
        ...
        <div v-else class="py-4 text-white text-sm font-medium">
          <span v-if="Object.keys($page.errors).length === 1">One fill error</span>
          <span v-else>The form contains {{ Object.keys($page.errors).length }} filling errors </span>
        </div>
      </div>
      <button type="button" class="group mr-2 p-2" @click="show = false">
