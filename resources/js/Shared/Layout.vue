      ...
      <div class="h-screen flex flex-col" @click="hideDropdownMenus">
        <div class="md:flex flex-shrink-0">
          <div class="bg-indigo-900 md:flex-shrink-0 md:w-56 px-6 py-4 flex items-center justify-between md:justify-center">
            <div class="mt-1 mr-4 text-white text-center">
              <inertia-link class="block py-2" href="/admin">
                DASHBOARD
              </inertia-link>
            </div>
          ...
          <div class="bg-white border-b w-full p-4 md:py-0 md:px-12 text-sm md:text-md flex justify-end items-center">
            <dropdown class="mt-1" placement="bottom-end">
              ...
              <div slot="dropdown" class="mt-2 py-2 shadow-xl bg-white rounded text-sm">
                <inertia-link class="block px-6 py-2 hover:bg-indigo-500 hover:text-white" :href="'/admin/users/edit/' +$page.auth.user.id">My profile</inertia-link>
                <inertia-link v-if="$page.auth.user.owner" class="block px-6 py-2 hover:bg-indigo-500 hover:text-white" href="/admin/users">Users</inertia-link>
                <inertia-link class="block px-6 py-2 hover:bg-indigo-500 hover:text-white" href="/admin/logout" method="post">Exit</inertia-link>
              </div>
            </dropdown>
          </div>
        </div>
        <div class="flex flex-grow overflow-hidden">
          <main-menu :url="url()" class="bg-indigo-800 flex-shrink-0 w-56 p-6 hidden md:block overflow-y-auto" />
